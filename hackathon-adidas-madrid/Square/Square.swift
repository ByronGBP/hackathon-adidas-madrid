...
class FieldArea: SCNNode {
    ...
    private var segments: [FieldArea.Segment] = []
    ...
    override init() {
        ...
        positioningNode.simdScale = float3(FieldArea.size * FieldArea.scaleForClosedSquare)
    ...
    private func performOpenAnimation() {
        ...
        SCNTransaction.animationTimingFunction = CAMediaTimingFunction(name: kCAMediaTimingFunctionEaseOut)
        SCNTransaction.animationDuration = FieldArea.animationDuration / 4
        positioningNode.simdScale = float3(FieldArea.size)
        SCNTransaction.commit()
    }
    ...
    private func performCloseAnimation(flash: Bool = false) {
        ...
        SCNTransaction.begin()
        SCNTransaction.animationTimingFunction = CAMediaTimingFunction(name: kCAMediaTimingFunctionEaseOut)
        SCNTransaction.animationDuration = FieldArea.animationDuration / 2
        ...
        if flash {
            let waitAction = SCNAction.wait(duration: FieldArea.animationDuration * 0.75)
            let fadeInAction = SCNAction.fadeOpacity(to: 0.25, duration: FieldArea.animationDuration * 0.125)
            let fadeOutAction = SCNAction.fadeOpacity(to: 0.0, duration: FieldArea.animationDuration * 0.125)
            fillPlane.runAction(SCNAction.sequence([waitAction, fadeInAction, fadeOutAction]))
            
            let flashSquareAction = flashAnimation(duration: FieldArea.animationDuration * 0.25)
            for segment in segments {
    ...
    private func scaleAnimation(for keyPath: String) -> CAKeyframeAnimation {
        ...
        let size = FieldArea.size
        let ts = FieldArea.size * FieldArea.scaleForClosedSquare
        ...
        scaleAnimation.duration = FieldArea.animationDuration
    ...
    private lazy var fillPlane: SCNNode = {
        ...
        material.diffuse.contents = FieldArea.fillColor
        material.isDoubleSided = true
        material.ambient.contents = UIColor.black
        material.lightingModel = .constant
        material.emission.contents = FieldArea.fillColor
